...
class GCViTLayer3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.downsample = None if not downsample else ReduceSize(dim=dim, norm_layer=norm_layer)
        self.q_global_gen = GlobalQueryGen(dim, input_resolution, window_size, num_heads)
        #self.time_mixer = TimeMixer(dim, 2 * dim)
        self.time_mixer = GCViTBlockTemporal(dim=dim,
                                            num_heads=num_heads,
                                            window_size=window_size,
                                            mlp_ratio=mlp_ratio,
                                            qkv_bias=qkv_bias,
    ...
    def forward(self, x, T):
        B, H, W, C = x.shape
        x = x.view(B // T, T, C, H, W).permute(0, 1, 3, 4, 2)
        x = self.time_mixer(x)
        #x = x.permute(0, 2, 1, 3, 4).view(B, C, H, W)
        x = x.view(B, H, W, C)
...
class GCViTLayerRecurent(nn.Module):
    """
    GCViT layer based on: "Hatamizadeh et al.,
    Global Context Vision Transformers <https://arxiv.org/abs/2206.09959>"
    """
    ...
    def forward(self, x, q_global_hidden=None):
        q_global = self.q_global_gen(_to_channel_first(x))
        #print('SHAPE forward_features q_global: ', q_global.shape, x.shape)

        if q_global_hidden is not None:
            q_global = self.norm(q_global + q_global_hidden)
        else:
            q_global = self.norm(q_global)
        
        q_global_hidden = q_global
        
        q_global = self.q_global_gen.reshape(q_global)

        for blk in self.blocks:
            x = blk(x, q_global)
        if self.downsample is None:
            return x, q_global_hidden
        return self.downsample(x), q_global_hidden

@BACKBONES.register_module()
class GCViTRecurent(nn.Module):
    """
    GCViT based on: "Hatamizadeh et al.,
    Global Context Vision Transformers <https://arxiv.org/abs/2206.09959>"
    """
    ...
    def __init__(self,
        ...
        self.levels = nn.ModuleList()
        for i in range(len(depths)):
            level = GCViTLayerRecurent(dim=int(dim * 2 ** i),
                                        depth=depths[i],
                                        num_heads=num_heads[i],
                                        window_size=window_size[i],
                                        mlp_ratio=mlp_ratio,
    ...
    def forward_features(self, x, hiddens):
        #print('SHAPE forward_features: ', x.shape)
        x = self.patch_embed(x)
        x = self.pos_drop(x)

        #print('SHAPE forward_features patch_embed: ', x.shape)

        for i in range(len(self.levels)):
            level, h = self.levels[i], hiddens[i]
            x, h = level(x, h)
            hiddens[i] = h
            #print('Hiddens: ', hiddens[i].shape)
