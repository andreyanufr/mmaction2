...
class GCViTBlockRecurrent(nn.Module):
    def __init__(self,
                 dim,
                 input_resolution,
                 num_heads,
...
class GCViTLayerRecurrent(nn.Module):
    def __init__(self,
                 dim,
                 depth,
                 input_resolution,
        ...
        # self.hw_conv1 = nn.Conv2d(dim, 2 * dim, 1, bias=False)
        # self.hw_conv2 = nn.Conv2d(2 * dim, 2 * dim, 3, 1, bias=False)
        # self.hw_conv3 = nn.Conv2d(dim, dim, 1, bias=False)
        # self.hw_act = nn.ReLU()
        # self.hw = nn.Sequential(
        #     nn.Conv2d(dim, 2 * dim, 1, bias=False),
        #     nn.ReLU(),
        #     nn.Conv2d(2 * dim, 2 * dim, 3, padding=1, bias=False),
        #     nn.ReLU(),
        #     nn.Conv2d(2 * dim, dim, 1, bias=False),
        # )
        # self.hw_act = nn.Sigmoid()
        ...
        # if input_resolution == 56:
        #     q_global_dim = int(input_resolution / 2 ** len(self.q_global_gen.to_q_global))
        # elif input_resolution == 28:
        #     q_global_dim = int(input_resolution / 2 ** len(self.q_global_gen.to_q_global))
        # elif input_resolution == 14:
        #     if window_size == 14:
        ...
        self.norm = norm_layer((q_global_dim, q_global_dim))
        #self.norm = norm_layer(dim)
        self.q_global_dim = q_global_dim

    def forward(self, x, q_global_hidden=None):
        q_global = self.to_q_global(x.view(-1,
                                           self.dim,
                                           self.resolution,
                                           self.resolution))
        ...
        # if q_global_hidden is None:
        #     q_global = q_global + self.hw(q_global)
        # else:
        #     q_global = q_global + self.hw(q_global_hidden)

        if q_global_hidden is not None:
            q_global = q_global + q_global_hidden
        
        q_global = self.norm(q_global)
        # q_global = self.norm(q_global.view(-1,
        #                                    self.q_global_dim,
        #                                    self.q_global_dim,
        #                                    self.dim))
        ...
        if self.downsample is None:
            return x, q_global
        return self.downsample(x), q_global
...
class GCViViT(nn.Module):
    def __init__(self,
        ...
        for i in range(len(depths)):
            level = GCViTLayerRecurrent(dim=int(dim * 2 ** i),
                               depth=depths[i],
...
@BACKBONES.register_module()
class GCViViTRecurrent(nn.Module):
    def __init__(self,
                 dim,
                 depths,
                 window_size,
    ...
    def forward(self, video):
        ...
        x = torch.stack(predictons, dim=2)#[:, :, -5:, :, :]
        x = self.avgpool(x)
        x = torch.flatten(x, 1)

        #print("SHAPE of gcvivit before permute: {}".format(x.shape))

        #x = x.permute(0, 2, 1, 3, 4).flatten(2)#[:, -3:, :]

        #print("SHAPE of gcvivit: {}".format(x.shape))

        return x


def gc_vit_xxtiny(pretrained=False, **kwargs):
    model = GCViViTRecurrent(depths=[2, 2, 6, 2],
                  num_heads=[2, 4, 8, 16],
...
def gc_vit_xtiny(pretrained=False, **kwargs):
    model = GCViViTRecurrent(depths=[3, 4, 6, 5],
                  num_heads=[2, 4, 8, 16],
                  window_size=[7, 7, 14, 7],
                  dim=64,
                  mlp_ratio=3,
...
def gc_vit_tiny(pretrained=False, **kwargs):
    model = GCViViTRecurrent(depths=[3, 4, 19, 5],
                  num_heads=[2, 4, 8, 16],
                  window_size=[7, 7, 14, 7],
                  dim=64,
                  mlp_ratio=3,
...
if __name__ == '__main__':
    ...
    num_features = int(64 * 2 ** 3)
    head = nn.Linear(num_features, 400)
    print('Num features: ', num_features)

    print("Completed. {}".format(y.shape))

    y = y.permute(0, 2, 1, 3, 4).flatten(2)

    print("Flatten. {}".format(y.shape))
